...
export default function FromUpdateProduct() {
  const { id } = useParams();
  const [imageFiles, setImageFiles] = useState<File[]>([]);
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState<string[]>([]);

  const productToUpdate: ProductType = useSelector((state: any) =>
    state.product.product.find(
  ...
  const [inputValue, setInputValue] = useState<ProductType>({
    ...
    imageProduct: productToUpdate.imageProduct,
  ...
  return (
    <div className="products_adminActions_part">
      <h3 className="create_product">Chỉnh sửa sản phẩm</h3>
